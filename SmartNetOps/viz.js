...
(function () {
  ...
  /* ── data centers (health by site) ────────────────────────── */
  function datacenters(el, model) {
    var items = (model && model.items) ? model.items : (Array.isArray(model) ? model : []);
    if (!items.length) {
      el.innerHTML = '<div class="dc-empty">No data center telemetry in this scope.</div>';
      return;
    }
    el.innerHTML = items.map(function (dc) {
      var health = dc.health;
      var tone = health == null ? "unk" : health >= 99 ? "ok" : health >= 95 ? "warn" : "crit";
      var width = health == null ? 0 : Math.max(0, Math.min(100, health));
      var label = String(dc.label || dc.id || "site");
      var meta = (dc.devices != null ? dc.devices + " devices" : "");
      var pct = health == null ? "—" : (Number.isInteger(health) ? health : health.toFixed(1)) + "%";
      return '<div class="dc-row" title="' + label.replace(/"/g, "&quot;") + ' · ' + pct + '">' +
        '<div class="dc-name">' + label + '</div>' +
        '<div class="dc-track"><i class="' + tone + '" style="width:' + width + '%"></i></div>' +
        '<div class="dc-val">' + pct + '</div>' +
  ...
  window.NetOpsViz = { sparkline: sparkline, Stream: Stream, gauge: gauge, heatmap: heatmap, topology: topology, stream: stream, datacenters: datacenters, colors: C };
