...
function testKpis() {
  ...
  assert.strictEqual(k.health.value, 91.6);
  assert.ok(k.health.available);
}

function testDatacenters() {
  const q = {
    bySiteDevices: ok([
      vec({ site_id: "SGP-SGP-DC-009" }, 20),
      vec({ site_id: "IND-MUM-DC-018" }, 8)
    ]),
    bySiteUp: ok([
      vec({ site_id: "SGP-SGP-DC-009" }, 20),
      vec({ site_id: "IND-MUM-DC-018" }, 6)
    ]),
    bySiteHealth: ok([vec({ site_id: "SGP-SGP-DC-009" }, 99.4)])
  };
  const dc = buildDatacenters(q);
  assert.strictEqual(dc.total, 2);
  assert.strictEqual(dc.items[0].id, "SGP-SGP-DC-009");
  assert.strictEqual(dc.items[0].health, 99.4);
  assert.strictEqual(dc.items[1].health, 75);

  const fromHealth = buildDatacenters({
    bySiteHealth: ok([
      vec({ site_id: "USA-NYC-DC-002" }, 97.2),
      vec({ site_id: "GBR-LON-DC-001" }, 88.1)
    ]),
    bySiteDevices: ok([vec({ site_id: "USA-NYC-DC-002" }, 14)])
  });
  assert.strictEqual(fromHealth.total, 2);
  assert.ok(fromHealth.items.some((d) => d.id === "GBR-LON-DC-001" && d.health === 88.1));
}
