...
function instantQueries(siteId) {
  ...
  return {
    ...
    bySiteHealth: `avg by (site_id) (${m("site_health_percent", siteId)})`,
    bySiteDevices: `sum by (site_id) (${m("site_devices_total:all", siteId)})`,
    bySiteUp: `sum by (site_id) (${m("site_devices_up:all", siteId)})`,
    bySiteMeraki: `sum by (site_id) (${m("meraki_site_devices_total", siteId)})`,
    bySiteDeviceInfo: `count by (site_id) (device_info${S})`
  };
}
...
function buildKpis(q) {
  ...
  const healthRec = { value: num(health, 2), unit: "%", available: health != null };
  return {
    health: healthRec,
...
function siteKey(metric) {
  if (!metric) return "";
  return String(metric.site_id || metric.site || "").trim();
}

function collectBySite(result, into) {
  samples(result).forEach((s) => {
    const id = siteKey(s.metric);
    if (!id) return;
    into[id] = s.value;
  });
}

function buildDatacenters(q) {
  const healthBy = {};
  const devicesBy = {};
  const upBy = {};
  collectBySite(q.bySiteHealth && q.bySiteHealth.result, healthBy);
  collectBySite(q.bySiteDevices && q.bySiteDevices.result, devicesBy);
  collectBySite(q.bySiteUp && q.bySiteUp.result, upBy);
  const merakiBy = {};
  collectBySite(q.bySiteMeraki && q.bySiteMeraki.result, merakiBy);
  Object.keys(merakiBy).forEach((id) => {
    if (devicesBy[id] == null) devicesBy[id] = merakiBy[id];
  });
  collectBySite(q.bySiteDeviceInfo && q.bySiteDeviceInfo.result, devicesBy);

  if (!Object.keys(devicesBy).length) {
    samples(q.deviceInfo && q.deviceInfo.result).forEach((s) => {
      const id = siteKey(s.metric);
      if (!id) return;
      devicesBy[id] = (devicesBy[id] || 0) + 1;
      if (s.value >= 1) upBy[id] = (upBy[id] || 0) + 1;
    });
  }

  const ids = new Set([
    ...Object.keys(healthBy),
    ...Object.keys(devicesBy),
    ...Object.keys(upBy)
  ]);
  const items = Array.from(ids).map((id) => {
    const devices = devicesBy[id];
    const up = upBy[id];
    let health = healthBy[id];
    if (health == null && devices) health = 100 * (up != null ? up : 0) / devices;
    return {
      id,
      label: String(id),
      devices: devices != null ? Math.round(devices) : null,
      up: up != null ? Math.round(up) : null,
      health: num(health, 1)
    };
  });
  items.sort((a, b) => (b.devices || 0) - (a.devices || 0) || String(a.label).localeCompare(String(b.label)));
  return {
    items: items,
    total: items.length
  };
...
async function getDashboard(siteId, client) {
  ...
  const datacenters = buildDatacenters(q);
  ...
  return {
    ...
    sparks: {
      health: series.health.map((p) => p[1]).slice(-24),
